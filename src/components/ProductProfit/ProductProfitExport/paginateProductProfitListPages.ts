...
export type ProductProfitListPage = {
  pageNo: number;
...
export type PaginateProductProfitListInput = {
  products: TProductProfitRow[];
  pageHeight: number;
  footerHeight: number;
  headerHeight: number;
...
export function paginateProductProfitListPages(
  rawInput: PaginateProductProfitListInput,
): ProductProfitListPage[] {
  const input = {
  ...
  const pages: ProductProfitListPage[] = [];
  ...
  const makePage = (
    pageProducts: TProductProfitRow[],
    isLast: boolean,
  ): ProductProfitListPage => ({
    pageNo,
