...
import { useGetMyActivityLogsQuery } from "@/redux/api/activityLogApi";
...
export default function ProfileActivityLogs() {
  ...
  const { data, isLoading } = useGetMyActivityLogsQuery({
    page,
    limit: 10,
  });
  ...
  return (
    <div className="bg-background rounded-lg border border-border p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-foreground">My Activity History</h2>
          <p className="text-sm text-muted-foreground">
            Audit logs and records of your actions in the system.
          </p>
        </div>
      </div>
