...
export type PaginateCustomerListInput = {
  customers: TCustomer[];
  pageHeight: number;
  footerHeight: number;
  headerHeight: number;
...
export function paginateCustomerListPages(
  rawInput: PaginateCustomerListInput,
): CustomerListPage[] {
  const input = {
  ...
  const overhead = (isFirst: boolean) =>
    ...
    (isFirst ? Math.max(input.headerHeight, 0) : Math.max(input.continuationBarHeight, 0)) +
    input.tableTopPadding +
    tableHeaderHeight;

  const makePage = (
    pageCustomers: TCustomer[],
    isLast: boolean,
  ): CustomerListPage => ({
    pageNo,
  ...
  while (pageNo <= 500 && remaining.length > 0) {
    ...
    const pageCustomers = remaining.slice(0, take);
    remaining = remaining.slice(take);
    const isLast = remaining.length === 0;
    pages.push(makePage(pageCustomers, isLast));
    startIndex += pageCustomers.length;
    pageNo += 1;
  }

  if (pages.length === 0) {
    pages.push(makePage(customers, true));
  } else {
