...
export type PaginateCustomerTransactionInput = {
  transactions: TCustomerTransaction[];
  pageHeight: number;
  footerHeight: number;
  headerHeight: number;
...
export function paginateCustomerTransactionPages(
  rawInput: PaginateCustomerTransactionInput,
  ...
  const pages: CustomerTransactionPage[] = [];
  ...
  const makePage = (
    pageTransactions: TCustomerTransaction[],
    isLast: boolean,
  ): CustomerTransactionPage => ({
    pageNo,
  ...
  while (pageNo <= 500 && remaining.length > 0) {
    ...
    const pageTransactions = remaining.slice(0, take);
    pages.push(makePage(pageTransactions, false));
