...
function ProductProfitExportPageInner() {
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 py-12 px-4">
        <div className="max-w-md mx-auto p-8 text-center space-y-4 rounded-xl border border-destructive/30 bg-card shadow-sm">
          <p className="text-destructive font-semibold">
            Failed to load the product profit report
          </p>
          <Link href="/product-profit">
            <Button variant="outline" size="sm" className="gap-2">
              <ArrowLeft className="size-4" /> Back to Product Profit
            </Button>
          </Link>
        </div>
      </div>
...
export default function ProductProfitExportPage() {
  return (
    <Suspense fallback={<ExportLoading />}>
      <ProductProfitExportPageInner />
    </Suspense>
  );
}
